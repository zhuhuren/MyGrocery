...
`;
code = code.replace('const CATEGORIES = [', globalVars + '\nconst CATEGORIES = [');

// 2. Replace CATEGORIES. with getAllCategories().
...
// 3. Replace location setup
const dynamicRenderLogic = `
function renderDynamicUI() {
  ...
  if(recLocs) {
    ...
    if(recLocs.firstElementChild) recLocs.firstElementChild.classList.add('active');
  }
...
`;
// Carefully inject before renderInventory
code = code.replace('async function renderInventory() {', dynamicRenderLogic + '\nasync function renderInventory() {');

// Remove the hardcoded event listeners
...
// Replace checkAuth safely
code = code.replace(/function checkAuth\(\) \{[\s\S]*?\}\s*function setupAuthEvents/m, checkAuthNew + '\n\nfunction setupAuthEvents');

// 5. Append settings JS
const settingsJs = `
// =======================
// Settings Logic
// =======================
...
document.getElementById('btn-add-location').addEventListener('click', () => {
  ...
  if (val) {
    if (!getAllLocations().includes(val.toLowerCase())) {
